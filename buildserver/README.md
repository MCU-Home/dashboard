...
The fat half of [ADR 0003](../docs/adr/0003-two-home-assistant-apps-dashboard-never-compiles.md)'s
two-App topology: a headless service that takes a resolved
`device-model.json` over the protocol of
[ADR 0006](../docs/adr/0006-build-service-protocol.md), compiles it with
the `mcuhome` builder, and hands the artifacts back.
...
> verifier ([ADR 0007](../docs/adr/0007-wire-content-and-credential-exposure.md)
...
of what a valid configuration is, on the side of the boundary that
[AGENTS.md](../AGENTS.md) says must not hold one.
...
produced, which is the anti-substitution anchor
[ADR 0010](../docs/adr/0010-flash-flow-ladder.md) needs.
...
wsl -d mcuhome-build -u root -e sh -c '
  cd /root/MCUHome/dashboard/buildserver &&
...
Packaging lives in the future packaging repo, not here (AGENTS.md). What
this package expects of it: `/data` for `jobs/`, `/share` mounted so the
