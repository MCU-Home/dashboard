...
#: Lowest ``mcuhome`` release this dashboard knows how to drive.
MCUHOME_VERSION_MIN = "0.1.0"
#: First ``mcuhome`` release this dashboard does *not* claim to support.
...
MCUHOME_VERSION_SPEC = f"mcuhome>={MCUHOME_VERSION_MIN},<{MCUHOME_VERSION_MAX_EXCLUSIVE}"
...
def check_mcuhome_version(version: str) -> None:
    ...
    raise IncompatibleBuilderError(
        ...
        f"{MCUHOME_VERSION_SPEC}, but mcuhome {version} is installed. "
